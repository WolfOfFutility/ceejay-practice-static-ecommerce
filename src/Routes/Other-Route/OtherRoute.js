import React from 'react'

import '../Monitor-Route/MonitorRoute.css'

import Background from '../../Components/Background-Component/Background'
import {Product} from '../../Components/Product-Component/Product'

function MonitorRoute() {
    return (
        <div className="Route">
            <Background id="Monitor" />
            <h2>Other Accessories</h2>
            <div className="product-list">
                <center>
                    <Product 
                        title='2 x 26" Monitors'
                        imageUrl="https://cdn.pixabay.com/photo/2016/03/09/09/17/computer-1245714_960_720.jpg"
                        price="523.60"
                    />
                    <Product 
                        title='2 x 26" Monitors'
                        imageUrl="https://cdn.pixabay.com/photo/2016/03/09/09/17/computer-1245714_960_720.jpg"
                        price="523.60"
                    />
                    <Product 
                        title='2 x 26" Monitors'
                        imageUrl="https://cdn.pixabay.com/photo/2016/03/09/09/17/computer-1245714_960_720.jpg"
                        price="523.60"
                    />
                    <Product 
                        title='2 x 26" Monitors'
                        imageUrl="https://cdn.pixabay.com/photo/2016/03/09/09/17/computer-1245714_960_720.jpg"
                        price="523.60"
                    />
                    <Product 
                        title='2 x 26" Monitors'
                        imageUrl="https://cdn.pixabay.com/photo/2016/03/09/09/17/computer-1245714_960_720.jpg"
                        price="523.60"
                    />
                    <Product 
                        title='2 x 26" Monitors'
                        imageUrl="https://cdn.pixabay.com/photo/2016/03/09/09/17/computer-1245714_960_720.jpg"
                        price="523.60"
                    />
                    <Product 
                        title='2 x 26" Monitors'
                        imageUrl="https://cdn.pixabay.com/photo/2016/03/09/09/17/computer-1245714_960_720.jpg"
                        price="523.60"
                    />
                    <Product 
                        title='2 x 26" Monitors'
                        imageUrl="https://cdn.pixabay.com/photo/2016/03/09/09/17/computer-1245714_960_720.jpg"
                        price="523.60"
                    />
                    <Product 
                        title='2 x 26" Monitors'
                        imageUrl="https://cdn.pixabay.com/photo/2016/03/09/09/17/computer-1245714_960_720.jpg"
                        price="523.60"
                    />
                    <Product 
                        title='2 x 26" Monitors'
                        imageUrl="https://cdn.pixabay.com/photo/2016/03/09/09/17/computer-1245714_960_720.jpg"
                        price="523.60"
                    />
                </center>
            </div>
        </div>
    )
}

export default MonitorRoute